...
import styles from './AddEditAlert.module.scss';
...
const ALERT_TYPES: IDropdownOption[] = [
  { key: 'Emergency',    text: 'Emergency' },
  { key: 'Warning/Advisory',      text: 'Warning / Advisory' },
  { key: 'Maintenance/Technical',       text: 'Maintenance / Technical' },
  { key: 'Operational/Administrative', text: 'Operational / Administrative' },
  { key: 'Security',         text: 'Security' },
  { key: 'Health/Safety',         text: 'Health & Safety' },
  { key: 'Suspension/Closure',         text: 'Suspension / Closure' },
  { key: 'StatusUpdate/Resolution',         text: 'Status Update / Resolution' },
];
...
const AddEditAlert: React.FC<AddEditAlertProps> = ({
  isOpen, mode, alert, context, onDismiss, onSave,
}) => {
  ...
  const [targetAudience, setTargetAudience] = useState<IAlertAudience[]>([]);
  const [bannerFile, setBannerFile] = useState<File | undefined>(undefined);
  const [bannerPreview, setBannerPreview] = useState<string | undefined>(undefined);
  const [newAttachments, setNewAttachments] = useState<File[]>([]);
  const [existingAttachments, setExistingAttachments] = useState<IAlertAttachment[]>([]);
  ...
  useEffect(() => {
    if (isOpen) {
      if (mode === 'edit' && alert) {
        ...
        setBody(alert.Body ?? '');
        setAlertType(alert.AlertType ?? 'Emergency');
        setPublishDate(alert.PublishDate ?? new Date());
        setExpiryDate(alert.ExpiryDate ?? undefined);
        setPriority(alert.Priority ?? 'Medium');
        setStatus((alert.Status === 'Draft' || alert.Status === 'Scheduled') ? alert.Status : 'Draft');
        setTargetAudienceType(alert.TargetAudienceType ?? 'All');
        setTargetAudience(alert.TargetAudience ?? []);
        setBannerPreview(alert.BannerImageUrl ?? undefined);
        setExistingAttachments(alert.Attachments ?? []);
      } else {
        setTitle('');
        setBody('');
  ...
  const validate = (): boolean => {
    ...
    if (isCritical && !expiryDate) newErrors.expiryDate = 'Expiry date is required for Critical alerts';
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      ...
      setSaveError('Failed to save alert. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className={styles.backdrop} onClick={handleBackdropClick} role="dialog" aria-modal="true">
      <div className={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        <div className={styles.panelContent}>
          ...
          <div className={styles.formSection}>
            ...
            <Dropdown
              ...
              options={ALERT_TYPES}
              onChange={(_, o) => setAlertType(o?.key as string ?? 'Emergency')}
              errorMessage={errors.alertType}
            />
          </div>
          ...
          <div className={styles.formSection}>
            ...
            {isCritical && (
              <MessageBar messageBarType={MessageBarType.severeWarning} className={styles.criticalWarning}>
                <strong>Critical Priority — Post Now!</strong> This alert will be published
                immediately upon saving, bypassing Draft and Scheduled status. It will supersede all
                other alerts in the carousel regardless of publish date. An expiry date is required.
              </MessageBar>
            )}
          </div>
          ...
          <div className={styles.formSection}>
            ...
            {targetAudienceType === 'Specific' && (
              <span className={styles.fieldHint}>
                Only the selected people below will see this alert.
              </span>
            )}
            {targetAudienceType === 'Except' && (
              <span className={styles.fieldHint}>
                Everyone will see this alert <strong>except</strong> the selected people below.
              </span>
            )}
            {targetAudienceType !== 'All' && (
              <>
                <PeoplePicker
                  ...
                  onChange={(items) => {
                    const people: IAlertAudience[] = (items || []).map((item: any) => ({ // eslint-disable-line @typescript-eslint/no-explicit-any
        ...
        <div className={styles.actionButtons}>
          ...
          <PrimaryButton
            text={isSaving ? 'Saving...' : mode === 'add' ? 'Add Alert' : 'Save Changes'}
            onClick={handleSave}
            disabled={isSaving}
          >
